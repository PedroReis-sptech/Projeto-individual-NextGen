/* ============================================================
   NextGen · utils.js — utilitários globais (frontend mock)
   ============================================================ */
...
// ── Sessão (localStorage mock) ───────────────────────────────
const Auth = {
  login(user) { localStorage.setItem('ng_user', JSON.stringify(user)); },
  logout()    { localStorage.removeItem('ng_user'); window.location.href = 'login.html'; },
  current()   {
    try { return JSON.parse(localStorage.getItem('ng_user')) || null; }
    catch { return null; }
  },
  require(tipo) {          // redireciona se não tiver permissão
